...
// LeetCode usernames are alphanumeric with underscores/hyphens. Reject anything
// else so we never interpolate arbitrary input into a GraphQL request.
const USERNAME_RE = /^[A-Za-z0-9_-]{1,39}$/;

async function handleLeetCode(url) {
	const username = url.searchParams.get('username') || 'yuvrajkarna';
	...
	// Pass the username as a GraphQL variable rather than string-interpolating it
	// into the query body.
	const query = `
  ...
  `;
	...
	return new Response(JSON.stringify(data), {
		headers: {
			...
			// Cache at the edge for 6h so repeat visitors don't hit LeetCode each time.
...
// Serve a handler's response from the edge cache when available, caching only
// successful responses so transient errors aren't pinned.
async function withEdgeCache(request, url, ctx, handler) {
...
export default {
	async fetch(request, env, ctx) {
		...
		if (pathname === '/github') {
			// Edge-cache to protect the GitHub rate limit.
			return withEdgeCache(request, url, ctx, () => handleGitHub(url, env));
		}

		// Default: / or /leetcode → LeetCode stats
		return withEdgeCache(request, url, ctx, () => handleLeetCode(url));
	},
};
